...
User submits task
    │
    ▼
[Privacy Scan] ── Presidio + regex detect PII / credentials
    │              Sensitive content kept locally by Leader; only sanitized
    │              version sent externally
    ▼
[Task Analysis] ── Leader LLM understands goal, constraints, deliverable format
    │
    ▼
[Task Planning] ── Generates dependency-ordered subtask list
    │               Each subtask gets an importance score (1–10)
    ▼
[Model Assignment] ── Matches each subtask to a worker via capability profile
    │                  + cost tier (local / mid / best)
    ▼
[Concurrent Execution] ── Independent subtasks run in parallel;
    │                       dependent ones wait for prerequisites
    │                       Each worker writes a heartbeat file every 10s
    │
    ├── [Monitor] Polls heartbeats → timeout detected → failover to backup model
    │
    ▼
[Review] ── Leader scores each subtask output (1–10); failed review triggers rework
    │
    ▼
[Integration] ── Leader merges all subtask results into a final deliverable
    │              Restores redacted placeholders from local mapping table
    ▼
[Memory Update] ── Updates model performance profiles, task history (SQLite),
                   RAG vector store, and infers user preference changes
```

### Leader Failover (Watchdog)

A lightweight Watchdog process runs alongside the main session and independently monitors the Leader's heartbeat file. If the Leader crashes (Ollama OOM, model load failure, etc.), the Watchdog detects it within 30 seconds and automatically:

1. Reads the latest session snapshot (`data/sessions/{id}/state.json`)
2. Iterates through the `leader.fallbacks` list in `config.yaml` to start a replacement Leader
3. The new Leader loads the snapshot, skips already-completed subtasks, and resumes from the breakpoint

### Cost Routing Logic

```
Subtask importance score (Leader-assigned, 1–10)
    < 5  →  local tier  → free local Ollama models
    5–7  →  mid tier    → best value-for-money paid model
    ≥ 8  →  best tier   → highest-quality model in history

Special case: monthly budget exhausted → all subtasks downgraded to local tier
```
...
Raw user input
    │
    ▼
Presidio scan + custom regex
    │
    ├── No sensitive content → sent directly to worker
    │
    └── Sensitive content found → sanitized version generated
                                   (replaced with [ENTITY_TYPE_xxxxx] placeholders)
            Leader keeps locally: original text + placeholder map
            External workers receive: sanitized version only
            After worker returns: Leader restores placeholders from map
```
...
ourAgentTeams/
├── config/
│   ├── config.yaml           # Main config: Leader, workers, monitor, cost, privacy
│   ├── models_profile.json   # Model capability profiles (auto-updated at runtime)
│   └── privacy_rules.yaml    # Custom sensitive-data detection rules
│
├── src/
│   ├── config.py             # Config loading, env-var injection, hot-reload
│   ├── cli/
│   │   ├── main.py           # All CLI command entry points
│   │   └── display.py        # Rich terminal UI components
│   ├── leader/
│   │   ├── orchestrator.py   # Core state machine — wires all modules together
│   │   ├── task_planner.py   # Task analysis and subtask decomposition
│   │   ├── model_selector.py # Capability matching + cost routing
│   │   ├── monitor.py        # Heartbeat polling + timeout failover
│   │   └── integrator.py     # Review scoring + result integration
│   ├── models/
│   │   ├── base.py           # Abstract worker interface
│   │   ├── local_model.py    # Ollama local model adapter
│   │   └── api_model.py      # LiteLLM unified cloud adapter
│   ├── memory/
│   │   ├── capability_store.py  # Model performance profiles (models_profile.json)
│   │   ├── task_history.py      # SQLite task history (SQLAlchemy ORM)
│   │   └── rag_engine.py        # ChromaDB vector retrieval
│   ├── cost/
│   │   └── calculator.py     # Token counting, cost estimation, routing decisions
│   ├── privacy/
│   │   └── guard.py          # Presidio + regex, sanitize / restore
│   ├── mcp/
│   │   └── server.py         # MCP tool registry (files, search, shell)
│   └── prompts/
│       ├── leader_system.md  # Leader system prompt (persona, responsibilities, constraints)
│       ├── task_analysis.md  # Task analysis prompt template
│       └── review.md         # Review scoring prompt template
│
├── data/                     # Runtime data (auto-generated)
│   ├── tasks/                # Subtask heartbeat files
│   ├── sessions/             # Session snapshots + Leader heartbeat
│   ├── memory/               # User preference profile (user_profile.json)
